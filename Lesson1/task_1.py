...
class AsyncPingHosts:
    ...
    async def _ping_host(self, address):
        ...
        proc = await create_subprocess_shell(f'ping {ping_key} 4 {address}', stdout=subprocess.DEVNULL)
    ...
    def get_ping_status_table(self):
        ...
        reachable = list(zip(self.reachable, repeat('Узел доступен')))
        unreachable = list(zip(self.unreachable, repeat('Узел недоступен')))

        # return tabulate(reachable + unreachable, headers, tablefmt="grid")
        return reachable + unreachable

if __name__ == '__main__':
    # создаем объекты адресов
    # FROM_ADDR = ip_address('10.0.1.1')
    # TO_ADDR = ip_address('10.0.1.15')
